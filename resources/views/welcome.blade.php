...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <title>{{ config('app.name', 'Laravel') }} - Solusi Digital Terpercaya</title>
    ...
    <body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased selection:bg-indigo-500 selection:text-white">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16 items-center">
                    <div class="flex items-center gap-2">
                        <flux:brand :name="config('app.name')" href="{{ route('home') }}" class="group">
                            <x-slot name="logo">
                                <x-app-logo-icon class="h-8 w-auto text-indigo-600 dark:text-indigo-400" />
                            </x-slot>
                        </flux:brand>
                    </div>
                    <div class="hidden md:flex items-center space-x-2 text-sm font-medium">
                        ...
                        @if (Route::has('login'))
                            @auth
                                <flux:button variant="primary" href="{{ route('dashboard') }}" class="rounded-full px-6">Dashboard</flux:button>
                            @else
                                <flux:button variant="ghost" href="{{ route('login') }}" class="text-indigo-600 dark:text-indigo-400">Log in</flux:button>
                            @endauth
                        @endif
                    </div>
                </div>
        ...
        <section id="home" class="relative min-h-screen flex items-center pt-20 overflow-hidden">
            <div class="absolute inset-0 z-0">
                ...
                <div class="absolute bottom-1/4 -right-20 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl animate-pulse delay-700"></div>
            </div>
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    <div class="space-y-8 animate-in fade-in slide-in-from-left-10 duration-1000">
                        <flux:heading size="3xl" level="1" class="tracking-tight leading-tight">
                            Wujudkan <span class="gradient-text">Ide Digital</span> Anda Menjadi Kenyataan
                        </flux:heading>
                        <flux:text size="lg" class="max-w-lg">
                            Kami membantu bisnis Anda berkembang melalui solusi teknologi modern, desain yang memukau, dan strategi digital yang tepat sasaran.
                        </flux:text>
                        <div class="flex flex-wrap gap-4">
                            <flux:button variant="primary" size="base" href="#contact" class="px-8 py-6 rounded-2xl shadow-xl shadow-indigo-500/30">
                    ...
                    <div class="hidden lg:block animate-in fade-in zoom-in duration-1000 delay-200">
                        <div class="relative group">
                            <div class="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-3xl blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
        ...
        <section id="services" class="py-24 bg-white dark:bg-slate-900/50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center space-y-4 mb-16">
                    <flux:heading size="sm" class="text-blue-600 dark:text-blue-400 font-semibold tracking-wider uppercase">Layanan Kami</flux:heading>
                    <flux:heading size="xl" level="2">Solusi Terbaik Untuk Bisnis Anda</flux:heading>
                </div>

                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($services as $service)
                        <flux:card class="group !p-8 !rounded-3xl hover:!border-indigo-500 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10">
                            <div class="w-14 h-14 bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center rounded-2xl mb-6 group-hover:scale-110 transition-transform">
                                @if($service->icon_image)
                                    <img src="{{ Storage::url($service->icon_image) }}" alt="{{ $service->title }}" class="w-8 h-8 object-contain">
                                @else
                                    <flux:icon name="wrench-screwdriver" class="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
                                @endif
                            </div>
        ...
        <section id="portfolio" class="py-24">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
                    <div class="space-y-4">
                        <flux:heading size="sm" class="text-purple-600 dark:text-purple-400 font-semibold tracking-wider uppercase">Portfolio</flux:heading>
                        <flux:heading size="xl" level="2">Karya-Karya Terbaru Kami</flux:heading>
                    </div>
                </div>
                    ...
                    @foreach($portfolios as $portfolio)
                        <div class="group relative overflow-hidden rounded-3xl bg-slate-200 dark:bg-slate-800">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent flex flex-col justify-end p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform">
                                <flux:text size="sm" class="!text-indigo-400 font-medium mb-2">{{ \Carbon\Carbon::parse($portfolio->project_date)->format('M Y') }}</flux:text>
        ...
        <!-- About Us Section -->
        @if($aboutPage)
            <section id="about" class="py-24 bg-indigo-600 text-white overflow-hidden relative">
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div class="grid lg:grid-cols-2 gap-16 items-center">
                        <div class="space-y-8">
                            <flux:heading size="sm" class="!text-indigo-200 font-semibold tracking-wider uppercase">Tentang Kami</flux:heading>
                            <flux:heading size="2xl" level="2" class="!text-white font-bold leading-tight">{{ $aboutPage->title }}</flux:heading>
                            <div class="text-indigo-100 lg:text-lg leading-relaxed space-y-4">
        ...
        <section id="contact" class="py-24 bg-white dark:bg-slate-950">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center space-y-4 mb-16">
                    <flux:heading size="sm" class="text-blue-600 dark:text-blue-400 font-semibold tracking-wider uppercase">Hubungi Kami</flux:heading>
                    <flux:heading size="xl" level="2">Mari Diskusikan Proyek Anda</flux:heading>
                </div>

                <div class="grid lg:grid-cols-5 gap-12">
                    <div class="lg:col-span-2 space-y-8">
                        <div class="flex gap-6 items-start">
                            <div class="w-12 h-12 bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center rounded-2xl shrink-0">
                                <flux:icon name="envelope" class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                            </div>
                            <div>
                                <flux:heading size="lg" class="mb-1">Email</flux:heading>
                                <flux:text>info@antigravity.id</flux:text>
                            </div>
                        </div>
                        <div class="flex gap-6 items-start">
                            <div class="w-12 h-12 bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center rounded-2xl shrink-0">
                                <flux:icon name="map-pin" class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                            </div>
                            <div>
                                <flux:heading size="lg" class="mb-1">Kantor</flux:heading>
                                <flux:text>Jl. Teknologi No. 123, Jakarta Selatan</flux:text>
                            </div>
                        </div>
                        <div class="flex gap-6 items-start">
                            <div class="w-12 h-12 bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center rounded-2xl shrink-0">
                                <flux:icon name="phone" class="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                            </div>
                            <div>
                                <flux:heading size="lg" class="mb-1">WhatsApp</flux:heading>
                                <flux:text>0812-3456-7890</flux:text>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-3">
                        <form action="{{ route('contact.store') }}" method="POST" class="space-y-6">
                            ...
                            <flux:card class="!p-8 !rounded-3xl shadow-xl">
                                ...
                                <flux:button type="submit" variant="primary" size="base" class="w-full !rounded-xl !py-4 !font-bold">
                                    Kirim Pesan
                                </flux:button>
                            </flux:card>
                        </form>
        ...
        <footer class="py-12 border-t border-slate-200 dark:border-slate-800 text-center">
            <div class="max-w-7xl mx-auto px-4">
                <flux:text size="sm">&copy; {{ date('Y') }} {{ config('app.name', 'Laravel') }}. All rights reserved.</flux:text>
            </div>
        </footer>
